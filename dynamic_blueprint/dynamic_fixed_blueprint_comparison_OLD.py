...
def compute_fixed_value_recurrent(initial_state: int, d: int) -> float:
...
def compute_optimal_value_recurrent(initial_state: int):
    print(f"Computing state {initial_state}")
    d_values = range(int(290/16), 10*num_scanners + 1)
    minimum_value = 1e9
    minimum_d = -1
    for d in d_values:
        value = compute_fixed_value_recurrent(initial_state, d)
...
def compute_fixed_value_function(I_max: int, d: int):
    ...
    # 1. Compute immediate costs vector
    ...
    # 2. Build Transition Probability Matrix P_d
    ...
    # 3. Solve the linear system: (I - beta * P_d) * V_d = c_d
    A = np.eye(I_max + 1) - discount * P_d
    V_d = np.linalg.solve(A, c_d)
    
    return V_d

def compute_optimal_value_function(d_values: List, max_state: int):
    d_values = np.arange()
    fixed_value_curves = []
...
if __name__ == "__main__":
    ...
    for init_state in states:
        ...
        for d in d_values:
            if 16 * d < init_state: 
                continue
            value = compute_fixed_value_stationary(d)
            if value < min_value:
                min_value = value
